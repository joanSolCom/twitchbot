...
if __name__== "__main__":
	...
	infile = sys.argv[1]
	fileHandler = codecs.open(infile, encoding='utf8', mode='r')
	channel = infile.split("_#")[1].split(".split.")[0]
	textOutput = ""
	i = 0
	msg = ""
	msgOk = ""
	for lines in fileHandler:
		...
		lines = lines.strip()
		#print(lines)
		if lines and 'PRIVMSG' in lines:
			try:			
				msg = lines.replace('\r', '').split("#"+channel+" :")[1]			
				if not msg.startswith('!') and not msg.startswith('http'):				
					msgOk = msg.strip()
					
			except IndexError:
				print("INDEX ERROR!!!")
	
		tokens = tokenizeRawTweetText(msgOk)
		#remove messages where the tokens are all the same (no needed for sem space)
		if not sameRepeatedToken(tokens):
			finalmsg = " ".join(tokens)
			if not finalmsg.startswith("ACTION") and not finalmsg.startswith("Added"):
				textOutput += finalmsg + '\n'
